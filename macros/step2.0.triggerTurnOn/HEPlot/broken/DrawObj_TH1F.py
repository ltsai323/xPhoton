...
def GetBinCenter(h) -> list:
    return h.to_hist().axes.centers[0]
def GetBinWidth(h:HistSet) -> list:
    return h.to_hist().axes.widths[0]
...
def DrawHIST(ax, hist, label:str, plotSTYLE:str):
    ax.hist
...
class DrawObj_TH1F:
    ...
    def Draw(self,ax):
        try:
            ...
            print('\n\n')
            raise IOError(e)
        DrawHIST(ax, hist, self.label, self.plotstyle)
