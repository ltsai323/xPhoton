...
Double_t* Ifit(float ptbin=13, int ebee=0, int fit_data=1, int jetbin=0)
{
  ...
  if(fit_data==1 || fit_data==11){ //1 norminal fit; 11 fit with toy template
    ...
    if(ptbin_int >=20) sprintf(hname,"data_%d_%d_%d_px2_chIso",ebee, jetbin, 19);
...
//   hbkg->SetLineWidth(2);
  ...
  if(ebee==0){
    sprintf(text,"|#eta^{#gamma}|<0.8, %.0f<p_{T}<%.0f GeV", ptbin_lowedge(ptbin_int),  ptbin_lowedge(ptbin_int+1));
  }else  if(ebee==1){
    sprintf(text,"0.8<|#eta^{#gamma}|<1.4442, %.0f<p_{T}<%.0f GeV", ptbin_lowedge(ptbin_int),  ptbin_lowedge(ptbin_int+1));
  }else if(ebee==2){
    sprintf(text,"1.566<|#eta^{#gamma}|<2.1, %.0f<p_{T}<%.0f GeV", ptbin_lowedge(ptbin_int),  ptbin_lowedge(ptbin_int+1));
  }else{
    sprintf(text,"2.1<|#eta^{#gamma}|<2.5, %.0f<p_{T}<%.0f GeV", ptbin_lowedge(ptbin_int),  ptbin_lowedge(ptbin_int+1));
  }
  tleg->SetHeader(text);
