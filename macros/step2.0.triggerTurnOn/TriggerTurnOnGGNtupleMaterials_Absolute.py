...
def EvtSelection(barrelORendcap):
    baseSelection = 'nPho>0 && nJet>0'
    if barrelORendcap == 'barrel':
        return '&&'.join([baseSelection, EvtSelection_BarrelPhoton()])
    if barrelORendcap == 'endcap':
        return '&&'.join([baseSelection, EvtSelection_EndcapPhoton()])
...
def HLTPassingInRange(tDIR, tTREE, hltBIT, barrelORendcap):
    ...
    tTREE.Draw(f'phoEt >> {hists["both"].GetName()}', '&&'.join([presel,PassPhoTrig(hltBIT), PassJetTrig()]))
    tTREE.Draw(f'phoEt >> {hists["base"].GetName()}', '&&'.join([presel                    , PassJetTrig()]))
    tTREE.Draw(f'phoEt >> {hists["trg" ].GetName()}', '&&'.join([presel,PassPhoTrig(hltBIT)               ]))
    if len(full_pt_range) > 0:
        tTREE.Draw(f'phoEt >> {hists["both_FullRange"].GetName()}', '&&'.join([presel,PassPhoTrig(hltBIT), PassJetTrig()]))
        tTREE.Draw(f'phoEt >> {hists["base_FullRange"].GetName()}', '&&'.join([presel                    , PassJetTrig()]))
...
if __name__ == "__main__":
    ...
    for hltBIT in range(MAX_HLTBIT+1): # check 0~8
