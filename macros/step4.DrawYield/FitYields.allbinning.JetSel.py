    ...
    for phoeta in range(2):
        for jeteta in range(2):
            for phopt in range(21):
                ...
                h_data=infile.Get('jetSel_data_%d_%d_%d_px1_chIso'%(phoeta,jeteta,phopt))
                h_sig =infile.Get( sigbin.naming('jetSel_gjet_%d_%d_%d_px1_chIso') )
                h_bkg =infile.Get( bkgbin.naming('jetSel_data_%d_%d_%d_px2_chIso') )
                ...
                space.DrawFitRes( sigbin, sigbin.naming('plots/jetSel_checking_%d_%d_%d.pdf'),pdfnames )
                mylog.debug( 'status ended: finished of a loop node')

    mylog.info('start writing fit fragments to root file')
    space.myWriteSpace('fitres_workspace_jetSel.root')
    del space
