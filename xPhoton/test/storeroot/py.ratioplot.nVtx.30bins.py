...
def drawrratios(targethist, compdict, output):
    ...
    upperpad=ratioplots[0].GetUpperPad().Clone()
    lowerpad=ratioplots[0].GetLowerPad().Clone()
    ...
    newcanv=ROOT.TCanvas('kk','',1200,1000)
    #newcanv.Clear('d')
    newcanv.cd()
    ratioplots[0].TopPad().Draw()
    #ratioplots[0].Draw()
    upperpad.Draw()
    lowerpad.Draw()
    newcanv.SaveAs(output)
