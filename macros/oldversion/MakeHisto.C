...
{
    char charaaaaa[50] = "";
    sprintf(charaaaaa,"Loop Event with %d events\n", intaaaaaa++);
    std::cout << charaaaaa;
...
// choose event by eta.
// 0 : -0.8<eta<0.8
// 1 : -1.5<eta<-0.8 or 0.8<eta<1.5
// 2 : -2.1<eta<-1.5 or 1.5<eta<2.1
// 3 : eta<-2.1 or eta>2.1
...
// setup an array and distinguish the pt belongs which bin.
// -1 : reject
// currently : pt > 15 =pass
Int_t MakeHisto::Ptbin(Float_t pt)
{
    ...
    if(pt<22.) return -1; // note: it is equalvalent to Pt<15
}
