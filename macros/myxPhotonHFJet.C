// copy from Rong Shyang's code. And modified with better indent.
#include <vector>
#include <TH1D.h>
...
//void usefulFuncs::StoreBoolInfoToHist( TH1* hist, int intBool ) { for ( int i=0; i<32; ++i ) if (CheckBoolInt(intBool, i)) hist->Fill(i); }
...
{
   TChain* mych = new TChain("ggNtuplizer/EventTree");
   for ( auto mypath : pathes ){ mych->Add(mypath.c_str()); }
   ...
   // output file contents building {{{
   TFile *fout_;
   fout_ = new TFile(oname,"recreate");

   TTree *outtree_;
   // asdf put histograms in folder.

   float ptcut[30] =
   {
      // 22, 30, 36, 50, 75, 90, 105,  120, 135, 150, 165, 175, 185,
      // 15, 20, 40, 60, 75, 90, 105,  120, 135, 150, 165, 175, 185,
      15, 20, 40, 60, 75, 90, 105,  120, 135, 150, 160, 170, 180,
   ...
   // book histograms {{{
   TH1F* h_phoSelection = new TH1F("phoSelRes", "Statistics plot for each indepent cut on photon ", 20, 0., 10. );
   TH1F* h_eleSelection = new TH1F("eleSelRes", "Statistics plot for each indepent cut on electron ", 20, 0., 10. );
   TH1F* h_jetSelection = new TH1F("jetSelRes", "Statistics plot for each indepent cut on jet ", 20, 0., 10. );
   TH1F* h_evtSelection = new TH1F("evtSelRes", "Statistics plot for each indepent cut on event", 20, 0., 10. );
   int phoSel, eleSel, jetSel, evtSel;
   //TH1F* h_mcPhotonSelIdx = new TH1F("mcPhoSelIdx", "Statistics plot for each indepent cut on gen photon ", 20, 0., 10. );
   TH1F* h_mcPhoSelection = new TH1F("mcPhoSelRes", "Statistics plot for each indepent cut on gen photon ", 20, 0., 10. );
   std::vector<int> mcPhoIdxs;
   std::vector<int> mcPhoSels;
   mcPhoIdxs.reserve(10);
   mcPhoSels.reserve(10);
   std::vector<int> truthMatchedRECOPhoton;
   TH1F* h_truthMatchedRECOPhoton = new TH1F("truthMatchedRecoPhoton", "Statistics plot for each reco photon with truth matching", 20, 0.,10.);
   TH1F* h_matchedGenPhoton = new TH1F("matchedGenPhoton", "statistics plot for gen photon used to matching to reco photon", 20,0.,10.);
   TH1F* h_matchedGenElectron = new TH1F("matchedGenElectron", "Statistics plot for gen electrons matching to reco photon", 20, 0., 10.);
   std::vector<int> matchedGenElectron;
   std::vector<int> matchedGenPhoton;

   /* evtSel :
    * 0: total event number
    * 1: num of jet < 1
    * 2: isData
    * 3: in data, no good vertex found.
    * 4: in data, no met filter found.
    * 5: has reco photon in event.
    */

   /* mcPhoSels :
    * 0: total gen photon number in a event
    * 1: mc pt > 15
    * 2: mc pt > 150, mcCalIsoDR04 filled.
    * 3: mcCalIsoDR04[k]<5.
    */

   /* truthMatchedRECOPhoton : record reco photon information which tries to matched with gen particles.
    * 0: total reco photon number in a event
    * 1: reco photon pt > 15.
    * 2: matched with genPhoton
    * 3: matched with genElectron
    * 
    */
   
   /* matchedGenPhoton :  record gen photon information which matched with a reco photon.
    * 0: #genPhoton dr<0.2 & dpt<0.2
    * 1: mcCalIsoDR04[k]<5.0
    */

   /* matchedGenElectron : record gen electron information which matched with a reco photon (converted photon.
    * 0: #genElectron with dr<0.2 & dpt<0.2
    * 1: #genElectron passed (mcCalIsoDR04[k]+mcTrkIsoDR04[k])<5.0
    */
   //TH1F* h_phoSelection = new TH1F("phoSelRes", "Statistics plot for each indepent cut on photon ", 64, 0., 32. );
   ...
   // asdf?? var = directly load from data.
   // asdf?? var_= weithed data.
   ...
   Float_t HoverE, sieie, chIso, phoIso, nhIso, chIsoRaw, phoIsoRaw, nhIsoRaw, chWorstIso;
   Float_t rho;
   Int_t phoFiredTrgs_, phohasPixelSeed_;

   Float_t sieip, sipip, e1x3, e2x2, e2x5, e5x5, rawE, scEtaWidth, scPhiWidth, esRR, esEn, mva, mva_hgg, photonIDmva;
   Int_t HggPresel;
   Float_t sieieFull5x5, sipipFull5x5, sieipFull5x5, e1x3Full5x5, e2x2Full5x5, e2x5Full5x5, e5x5Full5x5;
   Int_t isConverted;
   ...
   if(isMC==1)
   {
      ...
      puCalc.Init("src/external/puweights/102X/autum18/PU_histo_13TeV_2018_GoldenJSON_69200nb.root"); // asdf
      //puCalc.Init("/home/cmkuo/work/higgs/HZg/13TeV/ana/puweights/76X/PU_histo_13TeV_SilverJSON_74550nb.root");
   }

   // where to use this file? and why do we need this file? asdf
   // only used on mva photon selection
   //TFile* f = TFile::Open("external/transformation_76X_v2.root");
   TFile* fg = TFile::Open("src/external/transformation_76X_v2.root"); 
   // what's the usage of the tgr?
   TGraph *tgr[6];
   tgr[0] = (TGraph*) fg->Get("transfEtaWidthEB");
   tgr[1] = (TGraph*) fg->Get("transfS4EB");
   tgr[2] = (TGraph*) fg->Get("transffull5x5R9EB");

   tgr[3] = (TGraph*) fg->Get("transfEtaWidthEE");
   tgr[4] = (TGraph*) fg->Get("transfS4EE");
   tgr[5] = (TGraph*) fg->Get("transffull5x5R9EE");
...
evtSel = 0;
bool passEvt = false; 
evtSel+=usefulFuncs::SetBoolIntoInt(0);
...
//      std::cout <<"hi01\n";
      ...
      // directly load data and stored to tree {{{
//      std::cout <<"hi02\n";
      run     = data.GetInt("run");
//      std::cout <<"hi03\n";
...
//      std::cout <<"hi04\n";
      ...
      if(nJet <1 ) //asdf continue;
{ evtSel+=usefulFuncs::SetBoolIntoInt(1); passEvt = true; }
...
//      std::cout <<"hi04\n";
      ...
      {
//asdf
{ evtSel+=usefulFuncs::SetBoolIntoInt(2); passEvt = true; }
         ...
         if(!hasGoodVtx) //asdf continue;
{ evtSel+=usefulFuncs::SetBoolIntoInt(3); passEvt = true; }
         metFilters = data.GetInt("metFilters");
         if(metFilters != 0 ) //asdf continue;
{ evtSel+=usefulFuncs::SetBoolIntoInt(4); passEvt = true; }
...
//      std::cout <<"hi71\n";
      // directly load data and stored to tree end }}}

      //if(gjetSignal==0 &&  nPho==0) continue;  //skip entry if no recoPhoton

      // MC weighted variables {{{
      Float_t  pthat = 0;
      Int_t    nMC   = 0;
      Int_t*   mcPID =0;
      Int_t*   mcMomPID=0;
...
//      std::cout <<"hi08\n";
      if(!isData)
      {
         ...
         int ngenpho=0;
bool targetFound=false; //asdf need to be deleted.
mcPhoIdxs.clear(); mcPhoSels.clear();
         //Get gjet gen photon denominator
         for (Int_t k=0; k<nMC; ++k)
         {
            //asdf if (mcPID[k] == 22 &&  mcPt[k]>15. && TMath::Abs(mcMomPID[k]) <= 22)
if (mcPID[k] == 22 && TMath::Abs(mcMomPID[k]) <= 22)
{
mcPhoIdxs.push_back(k);
mcPhoSels.push_back(0);
int& photonSel = mcPhoSels.back();
photonSel+=usefulFuncs::SetBoolIntoInt(0);

if ( mcPt[k] > 15. ) photonSel+=usefulFuncs::SetBoolIntoInt(1);
            {
               if(mcPt[k]>150.) //asdf hmcGenIso->Fill(mcCalIsoDR04[k]);
{ photonSel+=usefulFuncs::SetBoolIntoInt(2); hmcGenIso->Fill(mcCalIsoDR04[k]); }

               float GENISO=0.; // GENISO = Pt sum of solid angled neighbor particles, with status 0~3. (basically mothers.)
               ...
               {	
                  // ngenpho++;
photonSel+=usefulFuncs::SetBoolIntoInt(3);
++ngenpho; targetFound=true;
if ( ngenpho==1 )
{
...
} //asdf additional if analysis
         }
         h_ngenpho->Fill(ngenpho);
      }
      // MC weighted variables end }}}
//      std::cout <<"hi09\n";



      //asdf if(nPho==0) continue; //skip entry if no recoPhoton
if(nPho==0) 
{ evtSel+=usefulFuncs::SetBoolIntoInt(5); passEvt = true; }

for ( int i : mcPhoSels ) usefulFuncs::StoreBoolInfoToHist(h_mcPhoSelection, i);
usefulFuncs::StoreBoolInfoToHist(h_evtSelection,evtSel);
if (!passEvt)
{

      // load varaibles used in further calculation at the event {{{
//      std::cout <<"hi11\n";
      Float_t *jetSubVtxPt;
      ...
      // Float_t* phoE1x35D           = data.GetPtrFloat("phoE1x3");
...
//      std::cout <<"hi11\n";
      ...
      {
         printf("-----------------------------------------------------------------------\n");
         printf("event %lli, #recoPhoton %d, #genParticle %d\n", event, nPho, nMC);
      }

//      std::cout <<"hi12\n";
      //count number of true photon
      int ntruephoton=0;
      // load varaibles used in further calculation at the event end }}}


      
      if(!isData) // matching recoPhoton & genPhoton {{{
      {
truthMatchedRECOPhoton.clear();
matchedGenPhoton.clear();
matchedGenElectron.clear();
         vector<int> mcid; // idx list of matched photon
         int nnMC=0; // number of matched photon. asdf
         // select photon decaying from parton and its pt>15. asdf
         for (Int_t k=0; k<nMC; ++k) // nMC : number of genParticle
         {
         ...
         vector<int> muonmcid;
         int nnmuonMC=0; // number of matched muon. asdf
         // select muon and its pt>20
         for (Int_t k=0; k<nMC; ++k)
         {
         ...
         // select electron
         vector<int> elemcid;
         int nneleMC=0; // number of matched electron. asdf
         for (Int_t k=0; k<nMC; ++k)
         {
            if (fabs(mcPID[k]) == 11 )
            {
         ...
         h_truepho->Fill(nnMC+0.001); // why 0.001 is added in the code? -- just prevent it falls into nnMC-1 category due to float precision. asdf
         h_convpho->Fill(nneleMC/2. + 0.001); // regard all electrons comes from convoluted photon asdf
         ntruephoton= nnMC + nneleMC/2.;

//         std::cout <<"hi13\n";
         //printf(" nMC photon %d, nME electron %d \n", nnMC, nneleMC);

         // use genPhoton to find caloIso in MC. : find the particles nearby the photon (by deltaR) and calculate the
         // i : idx of photon got from file. the photon MAY BE reconstructed from the CMS datastream. asdf


         for (Int_t i=0; i<nPho; ++i)  // asdf?? why is that you need to use deltaR to match a genPhoton and a genParticle?
         {
truthMatchedRECOPhoton.push_back(0);
int& truthMatchedRECOPhotonSel = truthMatchedRECOPhoton.back();
//bool passedRecoPhoton = false;
truthMatchedRECOPhotonSel+=usefulFuncs::SetBoolIntoInt(0);
            int asdfMYCHECK_phoMatchedCount = 0;
            int asdfMYCHECK_eleMatchedCount = 0;
            bool asdfMYCHECK_overwritten=false;
//            std::cout << nPho << "hi13.0001\n";
//            std::cout << "phoEt = " << phoEt[i] << std::endl;
            
            // if(phoEt[i]<15.) continue; // asdf
if ( phoEt[i] < 15. ) continue;
truthMatchedRECOPhotonSel+=usefulFuncs::SetBoolIntoInt(1);
//            std::cout <<"hi13.0002\n";
            isMatched    = -1;
            isMatchedEle = -1;
            isConverted = -1;
...
//            std::cout <<"hi13.01\n";
            for (int jj=0; jj<nnMC; ++jj) // jj : idx of genPhoton found from previous loop.
            { // asdf if the first candiate found, stopping this loop. Find the most pt candidate.
               int k = mcid[jj]; // k=(idx of genhoton in genParticle list)
               float dr = deltaR(phoEta[i], phoPhi[i], mcEta[k], mcPhi[k]);
               float dpt = fabs((phoEt[i] - mcPt[k])/mcPt[k]);

               // calculate the pt difference and deltaR value between genPhoton and recoPhoton
               if(dpt<0.2)hdR->Fill(dr);
               if(dr<0.2)hdpt->Fill(dpt);
               hmcCalIso->Fill(mcCalIsoDR04[k]);  // record the mcCalIso of genPhoton from ggAnalyzer asdf
               // asdf?? so what is mcCalIsoDR04? the definition is ? how to get this value?
               // asdf?? why the index is index in nMC? not nPho?
//// if the particles are next to the true photon, record the mcCalIsoDR04 value (which is obtained from ggAnalyzer) asdf
//               std::cout <<"hi13.02\n";
               ...
               if (dr < 0.2 && dpt < 0.2) // record particles nearby the genPhoton. Which is the reco deltaR <0.2 and reco ptDiff < 0.2. asdf
               {
matchedGenPhoton.push_back(0);
int& matchedGenPhotonCand = matchedGenPhoton.back();
matchedGenPhotonCand+=usefulFuncs::SetBoolIntoInt(0);
                  //float GENISO=0.; // GENISO is defined as the gen pt sum of (quarks,gluons and leptons) nearby the genPhoton. which is gen deltaR < 0.4. asdf
                  //// but not recorded asdf
                  //for (Int_t nn=0; nn<nMC; ++nn)
                  //{
                  //   if(nn == k ) continue;
                  //   if(fabs(mcPID[nn])>22 || mcStatus[nn]>3) continue; // asdf??? only quarks, gluons and leptons are left && status<=2 particles (decayed or final state particles)
                  //   float dr = deltaR(mcEta[k], mcPhi[k], mcEta[nn], mcPhi[nn]);
                  //   if( dr < 0.4) GENISO += mcPt[nn]; // asdf???????????????? what's this geniso usage?
                  //}
//                  std::cout <<"hi13.03\n";
                  // record the matching result. asdf
                  // But searching for FIRST candidate.
                  if(dr < 0.2 && dpt < 0.2 && mcCalIsoDR04[k]<5.0 ) //for gammajet photon pythia
                  {	
matchedGenPhotonCand+=usefulFuncs::SetBoolIntoInt(1);

                     if (asdfMYCHECK_phoMatchedCount > 1 ) cout << "---- warning --- there are " << asdfMYCHECK_phoMatchedCount << " gen particles matched with 1 reco photon\n";
                      if (asdfMYCHECK_phoMatchedCount++) continue;
                     ...
                     //break;
truthMatchedRECOPhotonSel+=usefulFuncs::SetBoolIntoInt(2);
                  }
               }

               
//               std::cout <<"hi13.04\n";
            } // for loop end of jj<nnMC

            // electron mc truth matching. asdf
            // asdf 1. this code overwrite the result found in previous loop, only last candidate kept.
            //      2. Also, information of photon candidate is exchanged to the last candidate of electron.
            //  if electron matched : meaning that a genElectron is reconstructed as a recoPhoton. Which means a photon but convert into ep pair.
...
if (dr < 0.2 && dpt < 0.2 )
{
matchedGenElectron.push_back(0);
int& matchedGenElectronCand = matchedGenElectron.back();

                  if (dr < 0.2 && dpt < 0.2 && ((mcCalIsoDR04[k]+mcTrkIsoDR04[k])<5.0 || doWmn==1) )
                  {
matchedGenElectronCand+=usefulFuncs::SetBoolIntoInt(1);
                     if ( asdfMYCHECK_eleMatchedCount++ ) continue;
                     if ( asdfMYCHECK_eleMatchedCount && asdfMYCHECK_phoMatchedCount ) asdfMYCHECK_overwritten = true;
                     isConverted = 1;
                     mcPt_  = mcMomPt[k];
                     mcEta_ = mcMomEta[k];
                     mcPhi_ = mcMomPhi[k];	
                     mcCalIso04_ = mcCalIsoDR04[k];
                     mcTrkIso04_ = mcTrkIsoDR04[k];
truthMatchedRECOPhotonSel+=usefulFuncs::SetBoolIntoInt(3);
                  }
}
               }
            } // for loop end of jj<nneleMC
//            std::cout <<"hi13.06\n";
            if(isMatched==1)
            {
               nmatch++;
...
//            std::cout <<"hi14\n";
            ...
            // my checking message asdf
            if (asdfMYCHECK_overwritten)
               cout << "--- information of matched photon is abandoned. the number of matched photon and electron are ("<<asdfMYCHECK_phoMatchedCount
                  <<", "<<asdfMYCHECK_eleMatchedCount<<"). Only the last matched electron candiate is recorded in the result.\n";
            else if ( asdfMYCHECK_phoMatchedCount >1 )
               cout << "---  " << asdfMYCHECK_phoMatchedCount << " matched photon found\n";
            else if ( asdfMYCHECK_eleMatchedCount >1 )
               cout << "---  " << asdfMYCHECK_eleMatchedCount << " matched ele    found\n";


         }

for ( int i : matchedGenPhoton ) usefulFuncs::StoreBoolInfoToHist(h_matchedGenPhoton, i);
for ( int i : matchedGenElectron) usefulFuncs::StoreBoolInfoToHist(h_matchedGenElectron, i);
for ( int i : truthMatchedRECOPhoton ) usefulFuncs::StoreBoolInfoToHist(h_truthMatchedRECOPhoton, i);
      ...
      vector<int> eleID;
      vector<int> muID;
      // asdf what is this definition?
      ElectronIDCutBased2015(data, 3, eleID); //0 veto, 1 loose, 2 medium, 3 tight

      h_nele->Fill(eleID.size());
      // depending on channel, prepare the lepP4[2] content, select the event with 2 IDENTIFIED leptons {{{
      // whose can be muon or electron. depending on the switch
      // which only selects the event with 2 IDENTIFIED leptons !! asdf
      // Once the electron have 3 or more leptons, select the first 2 electrons. (probabily highest pt electrons.
      if(doZee==1)
      {
      ...
      // depending on channel, prepare the lepP4[2] content end }}}


      // channel based ll reconstruction {{{
      if(doZee==1 && nLep_e>=2)
      {
      ...
      // asdf why you do this?
      // looking for the first electron from electron pool.
      // No matter this electron passed or failed the delta phi selection or not.
      // Why you need this ecandidate? it always to be 1
      int ecandidate=0;
      ...
      // HLT : leading jet containing photon passes the HLT. asdf
      //JETPD find leading jets fired trigger
      if(JETPD_PHOTONHLT==1)
      {
         for(int ijet=0; ijet<nJet; ijet++)
         {
            // find the first jet passing trigger. asdf
            if(jetFiredTrgs!=0)
            {
      ...
      // diphoton object reconstruction. asdf
      // selected photon passed PhotonPreselection. asdf
      if(isData==1)
      {
      ...
      for (Int_t i=0; i<nPho; ++i)
      {
         ...
         // asdf why you need to check a pointer as nullpointer? phoFiredTrgs
         ...
         if(!isData)
         {
            // once leading photon matched. asdf {{{
            if(i==0 && match[i]==1)
            {
               ...
               // check photon preselection
               if( PhotonPreselection(data, i, kFALSE) ==1)
               {
                  if(TMath::Abs(phoSCEta[i])<1.5)
                     hphoEB_pt_presel_nocsev->Fill(phoEt[i]);
                  else
                     hphoEE_pt_presel_nocsev->Fill(phoEt[i]);
            ...
            // once leading photon matched end. asdf }}}
         }
      ...
      for(int j=0; j<nJet; j++)
      {		
         //if( jetPFLooseId[j] && TMath::Abs(jetEta[j])<2.4 && jetPt[j]>30.)
         float jetjecunc = 1.;
         //float jetjecunc = 1.-jetJECUnc[j]; //1.;
         if(TMath::Abs(jetEta[j])<2.4 && jetPt[j]*jetjecunc>30.)
            //if(TMath::Abs(jetEta[j])<3. && jetPt[j]*jetjecunc>0.)
         {
      ...
      for (Int_t ii=0; ii<photon_requested; ii++)
      {
         ...
         if(doWmn==1)
         {
            ...
            else if(match[ipho]==1) hdR_pho_lep->Fill(phoP4.DeltaR(lepP4[0]),xsweight*genWeight);
         ...
         if(!isData)
         {
            ...
            {
               if (isMatched == 1)
                  cout<<isMatched<<" "<<isMatchedEle<<" "<<endl;
            }
            pthat_    = pthat;
            mcPt_     = mcpt[ipho];
            mcEta_    = mceta[ipho];
         ...
         if(isData==1 && doWmn==0 && qstar==1 && photonIDmva>0.4 && eleVeto==1 && TMath::Abs(phoSCEta[ipho])<1.4442 )
         {
            ...
            //     jetNCH[j] > 0 && jetNConstituents[j] > 1)
            //     {
            //     }
   ...
   // writing outputs block {{{
   fout_->cd();

   h_evtSelection->Write();
   h_mcPhoSelection->Write();
   h_truthMatchedRECOPhoton->Write();
   h_matchedGenPhoton->Write();
   h_matchedGenElectron->Write();
...
void xPhotonHFJet(Int_t dataset = 0)
{
   ...
   // if (dataset == 0)
   // {
   //   sprintf(fname, "/data6/ggNtuples/V08_00_26_06/job_summer16_gjet_pt20_MGG_40to80/*.root");
   //   //sprintf(fname, "/data6/ggNtuples/V08_00_11_01/job_spring16_gjet_pt20_MGG_40to80.root");
   //   //XS=154500 pb, filter eff 0.02155; nevent 9.80e7; eff_Int_L = N/XS/eff = 29426.382576 pb
   //   pathes.push_back(fname);
   //   XS=3.397e-02;//3.4701e-01;
   //   isMC=1;
   //   gjetSignal=1;
   // }
   // if (dataset == 1)
   // {
   //   sprintf(fname, "/data6/ggNtuples/V08_00_26_06/job_summer16_gjet_pt40_MGG_80toInf/*.root");
   //   pathes.push_back(fname);
   //   XS = 1.2052e-02; //XS=16792 pb - filter eff=0.0514 nevent 7.16e7 eff_Int_L = 82971pb
   //   isMC=1;
   //   gjetSignal=1;
   // }
   ...
   // if (dataset == 2)
   // {
   //   sprintf(fname, "/data6/ggNtuples/V08_00_26_06/job_summer16_gjet_pt20to40_MGG_80toInf/*.root");
   //   //sprintf(fname, "/data6/ggNtuples/V08_00_11_01/job_spring16_gjet_pt20to40_MGG_80toInf.root");
   //   pathes.push_back(fname);
   //   XS = 8.8364e-03;//3.6842e-02 ; //XS=137751 pb - filter eff=0.001587 nevt 2.47e+07; eff_Int_lumi = 113168.706628 pb
   //   isMC=1;
   //   gjetSignal=1;
   // }
   // if (dataset == 99)
   // {
   //   sprintf(fname, "/data6/ggNtuples/V08_00_26_06/job_summer16_gjet_pt15to6000/ggtree_mc_??.root");

   sprintf(fname, "/data533a/rslu/work/cms/ggNtuples/13TeV/mc/V10_02_10_04/job_autumn18_gjet_pt40_MGG_80toInf/GJet_Pt-40toInf_DoubleEMEnriched_MGG-80toInf_TuneCP5_13TeV_Pythia8/ggtree_mc_%d.root", dataset);
   pathes.push_back(fname);
   XS = 1.;
   isMC=1;
   gjetSignal=1;
