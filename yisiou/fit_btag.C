...
Double_t* Ifit(int PTBIN, float &w_L, float &w_C, float &w_B, char EBEE[10]="EB", int fit_data=1)//////Ifit
{
 ...
 //Get data from looping tree
  TFile *ftemplate = new TFile("./hist_btag.root");
  //TFile *ftemplate2 = new TFile("./hist_btag.root");
...
//   hbkg->SetError(yerr);
  ...
  char fname[30];
  sprintf(fname,"plots/fit_%d.png", 5);//////////////////////////////PTBIN
  c1->SaveAs(fname);
...
void pulltest(int ptbin=15, char EBEE[10]="EB", float input=0.5){
  ...
  h2->Draw();  
  sprintf(txt, "plots/fit_%d.png", 5);//////////////////PTBIN
  c2->SaveAs(txt);
...
int fit_btag(int argc = 0)
{
    //do the fit for particular ptbin (1~15):
    //root.exe .x 'fit_btag.C(5)'

    float w_L, w_C, w_B;
    float ar_L[15], ar_C[15], ar_B[15];
    if(argc != 0){
