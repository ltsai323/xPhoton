{
   ...
   Float_t eqLumi[10]={1.073e-01, 1.092e-01, 4.305e-01, 4.322e-01, 3.590e+00, 3.680e+00, 4.771e-02, 4.910e-02, 1.065e+01, 1.056e+01};
   Float_t weight[10];
...
for(int i = 0 ; i < t->GetEntries() ; i++){
  ...
  for(int j = 0 ; j < 15; j++){
	...
	h_mva_MC[j]->Fill(mva,weight[files]*w_is[j]*w_l[j]);
      if (mva > 0){
	h_jetpt_MC[j]->Fill(jetPt,weight[files]*w_is[j]*w_l[j]);
	h_recopt_MC[j]->Fill(recoPt,weight[files]*w_is[j]*w_l[j]);
	if(isMatched==1){
	  if(flavour == 0) {
	    h_jetpt_LM[j]->Fill(jetPt,weight[files]*w_is[j]*w_l[j]);
            h_recopt_LM[j]->Fill(recoPt,weight[files]*w_is[j]*w_l[j]);
	  }else if (flavour == 4) {
	    h_jetpt_CM[j]->Fill(jetPt,weight[files]*w_is[j]*w_c[j]);
            h_recopt_CM[j]->Fill(recoPt,weight[files]*w_is[j]*w_c[j]);
	  }else{
	    h_jetpt_BM[j]->Fill(jetPt,weight[files]*w_is[j]*w_b[j]);
	    h_recopt_BM[j]->Fill(recoPt,weight[files]*w_is[j]*w_b[j]); 
	  }		
	}else{
	  if(flavour == 0) {
	    h_jetpt_LN[j]->Fill(jetPt,weight[files]*w_is[j]*w_l[j]);
	    h_recopt_LN[j]->Fill(recoPt,weight[files]*w_is[j]*w_l[j]);
	  }else if (flavour == 4) {
	    h_jetpt_CN[j]->Fill(jetPt,weight[files]*w_is[j]*w_c[j]);
	    h_recopt_CN[j]->Fill(recoPt,weight[files]*w_is[j]*w_c[j]);
	  }else{
	    h_jetpt_BN[j]->Fill(jetPt,weight[files]*w_is[j]*w_b[j]);
	    h_recopt_BN[j]->Fill(recoPt,weight[files]*w_is[j]*w_b[j]);
	  }
	}
      }	
    }
