...
TH1F* hTrg = (TH1F*) fTrg->Get( Form("qcd_%d_%d_%d_px1_chIso",pEtaBin,jEtaBin,pPtBin) );
TH1F* hCmp = (TH1F*) fCmp->Get( Form("photonFit_%d_%d_%d/fake_signalRegion",pEtaBin,jEtaBin,pPtBin) );
...
for ( int jEtaBin = 0; jEtaBin < 2; ++jEtaBin ) {
for ( int pPtBin = 0; pPtBin < 21; ++pPtBin ) {
TH1F* hTrg = (TH1F*) fTrg->Get( Form("qcd_%d_%d_%d_px2_chIso",pEtaBin,jEtaBin,pPtBin) );
TH1F* hCmp = (TH1F*) fCmp->Get( Form("photonFit_%d_%d_%d/fake_sidebandRegion",pEtaBin,jEtaBin,pPtBin) );
...
for ( int jEtaBin = 0; jEtaBin < 2; ++jEtaBin ) {
for ( int pPtBin = 0; pPtBin < 21; ++pPtBin ) {
TH1F* hTrg = (TH1F*) fTrg->Get( Form("qcd_%d_%d_%d_all",pEtaBin,jEtaBin,pPtBin) );
TH1F* hCmp = (TH1F*) fCmp->Get( Form("photonFit_%d_%d_%d/fake_allRegion",pEtaBin,jEtaBin,pPtBin) );

Double_t nTrg = hTrg->Integral(1, hTrg->GetNbinsX()+1);
Double_t nCmp = hCmp->Integral(1, hCmp->GetNbinsX()+1);
nTrg=hTrg->GetEntries();
nCmp=hCmp->GetEntries();
totEntriesTrg+=hTrg->GetEntries();
totEntriesCmp+=hCmp->GetEntries();
Double_t diff = nTrg-nCmp;
if ( fabs(diff) > 1e-1 ) {
printf("At bin (%d,%d,%2d) : trg = %10.2f and cmp = %10.2f. Diff = %8.2f in %4.2f%%\n",pEtaBin,jEtaBin,pPtBin, nTrg,nCmp,diff, 100. * diff / nTrg);
}
