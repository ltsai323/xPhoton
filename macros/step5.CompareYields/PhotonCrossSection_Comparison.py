...
def MyLegend(hists) -> ROOT.TLegend:
    ...
    return leg
def MyLegend_Sep(hists,refhists) -> ROOT.TLegend:
    leg = ROOT.TLegend(0.29, 0.71, 0.90, 0.92)
    leg.SetFillColor(0)
...
if __name__ == '__main__':
    ...
    for pEtaBin in range(2):
        for jEtaBin in range(2):
            ...
            hist.SetLineColor(histcolors)
            hist.SetMarkerColor(histcolors)
            hist.Draw('EP')
    ...
    zoomfactor = 1.
    for hist, refhist in zip(reversed(hists),reversed(refhists) ):
    ...
    for hist,refhist in zip(hists,refhists):
        ...
        hist.Draw('EP SAME')
    leg_sep = MyLegend_Sep(hists,refhists)
    leg_sep.Draw()
