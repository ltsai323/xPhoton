...
def DrawFitRes( figname, space, dataset, fitres, canv=ROOT.TCanvas('c0','',1200,1000)):
    ...
    sigratio=Nsig.getVal()/(Nsig.getVal()+Nbkg.getVal())
    leg=ROOT.TLegend(0.25, 0.55, 0.75, 0.85, '#splitline{%s - %s - %s}{Signal purity = %.2f - chi2/nDof = %.1f/9}'%(phoetaregion,jetetaregion,phoptregion, sigratio, chi2Val) )
...
if __name__ == '__main__':
    ...
    for phoeta in range(2):
        for jeteta in range(2):
            for phopt in range(2):
                PHOPTBIN=phopt
                PHOETABIN=phoeta
                JETETABIN=jeteta

                h_data=infile.Get('data_%d_%d_%d_px2_chIso'%(phoeta,jeteta,phopt))
                h_sig =infile.Get('gjet_%d_%d_%d_px2_chIso'%(phoeta,jeteta,phopt))
                #h_bkg =infile.Get('data_%d_%d_%d_px2_chIso'%(phoeta,jeteta,phopt))
                exphoptbin= phopt if phopt < 17 else 17
                h_bkg =infile.Get( 'qcd_%d_%d_%d_px2_chIso'%(phoeta,jeteta,exphoptbin))
                ...
                DrawFitRes('plots/check_%d_%d_%d.png'%(phoeta,jeteta,phopt),
