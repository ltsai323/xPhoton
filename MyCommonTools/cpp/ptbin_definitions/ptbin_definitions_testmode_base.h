#ifndef __ptbin_definitions_testmode_base_h__
#define __ptbin_definitions_testmode_base_h__
#include <vector>
#include <string>

static int testmode = 5;
...
// merge bin 135-175 and 175-190. All they used HLT120 . And merge 190-220-250 }}}
// testmode 4 {{{
std::vector<float> ptbin_ranges_2016_mergedBin_test4()
{ return std::vector<float>({25,34,40,56,70,85,100,115,135,155,190,200,220,300,400,500,750,1000}); }

Int_t TriggerBit_2016_mergedBin_test4(Int_t ptbin)
{
    std::vector<int> bitdef({ 0, 1, 2, 3, 3, 4,  5,  5,  6,  6,  7,  7,  7,  7,  8,  8,  8,   8});
    if ( ptbin < 0 ) return -1;
    if ( ptbin >= bitdef.size() ) return -1;
    return bitdef[ptbin];
...
// 2016 binning end }}}
// testmode 5 {{{
std::vector<float> ptbin_ranges_2016_mergedBin_test5()
{ return std::vector<float>({25,34,40,56,70,85,100,115,135,    190,200,220,300,400,500,750,1000}); }

Int_t TriggerBit_2016_mergedBin_test5(Int_t ptbin)
{
    std::vector<int> bitdef({ 0, 1, 2, 3, 3, 4,  5,  5,  6,      7,  7,  7,  7,  8,  8,  8,   8});
    if ( ptbin < 0 ) return -1;
    if ( ptbin >= bitdef.size() ) return -1;
    return bitdef[ptbin];
...
// 2016 binning end }}}
// testmode 9 {{{
std::vector<float> ptbin_ranges_2016_mergedBin_test9()
{ return std::vector<float>({190,200,220,250,300,350,400,500,750,1000}); }

Int_t TriggerBit_2016_mergedBin_test9(Int_t ptbin)
{
    std::vector<int> bitdef({  7,  7,  7,  7,  7,  7,  7,  7,  7,   7});
    if ( ptbin < 0 ) return -1;
    if ( ptbin >= bitdef.size() ) return -1;
    return bitdef[ptbin];
...
//    if ( testmode ==  4 ) return ptbin_ranges_2016_mergedBin_test4();
//    if ( testmode ==  5 ) return ptbin_ranges_2016_mergedBin_test5();
//    return std::vector<float>();
//    //return ptbin_ranges_Unprescaled();
//}
std::vector<float> ptbin_ranges_base(int testMODE)
{
    if ( testMODE == -1 ) return ptbin_ranges_2016();
    if ( testMODE ==  0 ) return ptbin_ranges_2016_mergedBin();
    if ( testMODE ==  1 ) return ptbin_ranges_2016_mergedBin_test1(); // merge 190
    ...
    if ( testMODE ==  4 ) return ptbin_ranges_2016_mergedBin_test4();
    if ( testMODE ==  5 ) return ptbin_ranges_2016_mergedBin_test5();
    if ( testMODE ==  9 ) return ptbin_ranges_2016_mergedBin_test9();
    return std::vector<float>();
    //return ptbin_ranges_Unprescaled();
}
...
//        if ( testmode ==  4 )return TriggerBit_2016_mergedBin_test4(ptbin); // merge 175
//        if ( testmode ==  5 )return TriggerBit_2016_mergedBin_test5(ptbin);
...
//}
Int_t TriggerBitBase( const std::string& dataera, Int_t ptbin, int testMODE){
    // used for ptbin_ranges_2016_mergedBin()
    if ( dataera == "2016ReReco" ||
         dataera == "UL2016PreVFP" ||
         dataera == "UL2016PostVFP" )
    {
        ...
        if ( testMODE ==  4 )return TriggerBit_2016_mergedBin_test4(ptbin); // merge 175
        if ( testMODE ==  5 )return TriggerBit_2016_mergedBin_test5(ptbin);
        if ( testMODE ==  9 )return TriggerBit_2016_mergedBin_test9(ptbin);
        //return TriggerBit_UnPrescaled(ptbin);
        return -1;
    }
    if ( dataera == "UL2017" )
