#define MakeBinning_cxx
#include "MakeBinning.h"
#include <TH1.h>
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
struct outputTree
{
    Int_t HLTBin, phoEtaBin, jetEtaBin, phoPtBin, phoMatchedStatus, jetFlvrBin, parity;
    Int_t isSignalRegion, isSidebandRegion;
    Float_t pt, mva, calb_mva, photonIDmva, eventweight, chIsoRaw, phoIsoRaw, chPlusphoIso, chWorstRaw,
            btagSystUp, btagSystCentral, btagSystDown,
    ...
    void Clear() {}
    void RegTree(TFile* f) {}
    void Write() {}
    TTree* t;
};

void MakeBinning::Loop(Int_t extracut = 0)
{
...
Int_t MakeBinning::EBEE(Float_t eta)
{
    if ( TMath::Abs(eta)<1.4442 ) return 0;
    else                          return 1;
}
Int_t MakeBinning::Ptbin(Float_t pt)
{
...
// unused
Int_t MakeBinning::HLTbit(Float_t pt){
...
Int_t MakeBinning::JetEtaBin(Float_t eta) {  
    if(jetPt==0) return 2;
    if(TMath::Abs(eta)<1.5) return 0;
    return 1;
}
Int_t MakeBinning::triggerbit(Int_t ptbin){
...
Int_t MakeBinning::JetFlavourBin( int jethadflvr )
{
