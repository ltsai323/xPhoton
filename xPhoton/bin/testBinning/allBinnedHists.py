...
NUMBIN_EVTPARITY=2
NUMBIN_JETFLVR=3
def CreateBinnedHists(oFile,bin_phopt, bin_phoeta, bin_jeteta):
    sel_binning=[
            'bin_phopt == %d'   % bin_phopt,
            'bin_phoeta == %d'  % bin_phoeta,
            'bin_jeteta == %d'  % bin_jeteta,
            ]
    # use calib_chIso
    #sel_SG='region_pho == 1'
    #sel_SB='region_pho ==-1'
    ...
    oDir=oFile.mkdir( 'photonFit_%d_%d_%d' % (bin_phoeta,bin_jeteta,bin_phopt) )
...
if __name__ == "__main__":
    ...
    NUM_PHOETABIN=2
    NUM_JETETABIN=2
    for pPtBin in range(NUM_PHOPTBIN):
        for pEtaBin in range(NUM_PHOETABIN):
            for jEtaBin in range(NUM_JETETABIN):
                    CreateBinnedHists(oFile,pPtBin,pEtaBin,jEtaBin)
    oFile.Close()
