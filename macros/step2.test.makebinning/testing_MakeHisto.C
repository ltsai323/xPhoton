...
void MakeHisto::Loop(Int_t extracut = 0)
{
    ...
    h_chiso_sg->Write();
    h_chworst_sg->Write();
    for ( int bin_phoeta = 0; bin_phoeta < NUMBIN_PHOETA; ++bin_phoeta )
        for ( int bin_jeteta = 0; bin_jeteta < NUMBIN_JETETA; ++bin_jeteta )
            for ( int bin_phopt = 0; bin_phopt < NUMBIN_PHOPT; ++bin_phopt )
                fout->mkdir( Form("bin_%s", Form("%d_%d_%d",bin_phoeta,bin_jeteta,bin_phopt) ) );
