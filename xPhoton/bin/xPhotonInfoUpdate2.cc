...
struct JsonInfo
{
    JsonInfo( const char* jsonfile )
    {
        ...
        for ( pt::ptree::value_type& ifile : root.get_child("xmlFiles") )
            xmlFiles.emplace_back( ifile.second.data() );
    }
...
void UpdateMVA( const JsonInfo& jobInfo, TTree* inTree )
{
    ...
    for ( int iBE = 0; iBE < 2; ++iBE )
    {
        ...
        // testing
        const char* xmlFile = jobInfo.xmlFiles[iBE].c_str();
        printf("PhotonMVAcalculator[%s] using %s\n", iBE?"Endcap":"Barrel", xmlFile);
        tmvaReader[iBE]->BookMVA("BDT", xmlFile );
    }
    // declare MVA done }}}
